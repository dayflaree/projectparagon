...
function PANEL:Init()
    if (IsValid(ix.gui.vanguardMenu)) then
        ix.gui.vanguardMenu:Remove()
    end

    ix.gui.vanguardMenu = self
    ...
    local close = self.buttons:Add("ixVanguardMenuButton")
...
function PANEL:GetStandardSubpanelSize()
    return ScrW() * 0.8 - self:GetPadding() * 3, ScrH() - self:GetPadding() * 2
end
...
function PANEL:SetupTab(name, info, sectionParent)
    ...
    if (sectionParent) then
        ...
        button = self.tabs:Add("ixVanguardMenuSelectionButton")
...
function PANEL:PopulateTabs()
    ...
    hook.Run("CreateVanguardMenuButtons", tabs)
    ...
    if (ix.gui.lastMenuTabVanguard) then
        for i = 1, #self.tabs.buttons do
            local button = self.tabs.buttons[i]

            if (button.id == ix.gui.lastMenuTabVanguard) then
                default = button
                break
            end
        end
...
vgui.Register("ixVanguardMenu", PANEL, "ixVanguardSubpanelParent")

if (IsValid(ix.gui.vanguardMenu)) then
    ix.gui.vanguardMenu:Remove()
end

ix.gui.lastMenuTabVanguard = nil

concommand.Add("ix_vanguard_menu", function()
    if (IsValid(ix.gui.vanguardMenu)) then
        ix.gui.vanguardMenu:Remove()
    else
        vgui.Create("ixVanguardMenu")
    end
end)