...
ix.lang.AddTable("english", {
	...
	optdThirdpersonSmoothness = "How smooth should the thirdperson camera be?",
})
...
if (CLIENT) then
	...
	ix.option.Add("thirdpersonSmoothness", ix.type.number, 10, {
		category = "thirdperson", min = 1, max = 50,
		hidden = isHidden
	})
	...
	function PLUGIN:CalcView(client, origin, angles, fov)
		...
		if (client:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer()) then
			...
			if ( ix.option.Get("thirdpersonSmoothPosition", true) ) then
				if not ( oldPos ) then
					oldPos = origin
				end
			end

			if ( ix.option.Get("thirdpersonSmoothAngle", true) ) then
				if not ( oldAng ) then
					oldAng = angles
				end
			end
			...
			if ( ix.option.Get("thirdpersonSmoothPosition", true) ) then
				oldPos = LerpVector(ft * ix.option.Get("thirdpersonSmoothness", 10), oldPos, view.origin)

				view.origin = oldPos
			end

			if ( ix.option.Get("thirdpersonSmoothAngle", true) ) then
				oldAng = LerpAngle(ft * ix.option.Get("thirdpersonSmoothness", 10), oldAng, view.angles)
