...
function ITEM:RemoveOutfit(ply)
    ...
    if (self.newFaction) then
        if (char:GetData("oldFaction")) then
            char:SetFaction(char:GetData("oldFaction", FACTION_CITIZEN))
            char:SetData("oldFaction", nil)
            char:SetData("faction", char:GetFaction())
        else
...
ITEM.functions.Equip = {
    ...
    OnRun = function(item, creationClient)
        ...
        if (item.newFaction) then
            char:SetData("oldFaction", item.player:GetCharacter():GetFaction())
            char:SetData("faction", item.newFaction)

            item.player:GetCharacter():SetFaction(item.newFaction)
        end
...
function ITEM:CanEquipOutfit()
    if (self.maxArmor) then
        local bgItems = self.player:GetCharacter():GetInventory():GetItemsByBase("base_newoutfits", true)
