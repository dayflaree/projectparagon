...
-- Add config to allow chat character limit customization
ix.config.Add("chatMax", 1000, "Maximum length of a chat message.", nil, {
...
if (CLIENT) then
    ...
    net.Receive("ixChatMessage", function(length, client)
        ...
        if ((client.ixNextChat or 0) < CurTime() and isstring(text) and text:find("%S")) then
            local maxLength = ix.config.Get("chatMax", 1000)

            if (text:utf8len() > maxLength) then
                text = text:utf8sub(1, maxLength)
            end
