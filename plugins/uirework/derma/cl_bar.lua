...
function PANEL:Init()
	self:SetSize(ScrW() * 0.25, ScrH())
	self:SetPos(ScrW() - ScrW() * 0.25 - 10, 10)
	self:ParentToHUD()
...
function PANEL:AddBar(index, color, priority, icon)
	...
	panel:SetSize(self:GetWide(), ScreenScale(8))
	panel:SetVisible(false)
	panel:SetID(index)
	panel:SetColor(color)
	panel:SetPriority(priority)
	panel:SetIcon(icon or "minerva/icons/star.png")
...
function PANEL:Init()
	...
	self.icon:SetMaterial("minerva/icons/star.png")
	self.icon:SetSize(ScreenScale(8), ScreenScale(8))
	self.icon:Dock(LEFT)
	self.icon:DockMargin(0, 0, 5, 0)

	self.bar = self:Add("DPanel")
	self.bar:Dock(FILL)
	self.bar:DockMargin(5, 5, 0, 5)
	self.bar.Paint = function(this, width, height)
		...
		self.icon:SetImageColor(ColorAlpha(self.color, 255))

		surface.SetDrawColor(ColorAlpha(self.color or color_white, 255))
