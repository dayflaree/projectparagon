...
ix.config.Add("allowPersonalFootstepVolume", false, "Whether people can change the volume of their own footsteps.", nil, {
	category = "Footsteps"
})
...
if CLIENT then
	...
	function GAMEMODE:GetNextStepTime(client, vel)
		...
		if client:WaterLevel() >= 1 then
			...
			if client:KeyDown(IN_WALK) and vel < 45 then
				val = 0.375 / (vel / 45)
			else
				val = math.max(math.Remap(vel, 60, 235, 0.6, 0.3), 0.1)
			end
		end
	...
	function GAMEMODE:HandlePlayerStep(client, side)
		local snd, volume, level = hook.Run("GetDefaultStepSound", client, side)
		...
		EmitSound(data.snd, client:GetPos(), client:EntIndex(), CHAN_AUTO, volume, level)
