...
do
    ix.bar.Add(function()
        local injureText, injureTextColor = hook.Run("GetInjuredText", LocalPlayer())

        return math.max(LocalPlayer():Health() / LocalPlayer():GetMaxHealth(), 0), injureText
    end, Color(200, 50, 40), nil, "health", "90/projectparagon/ui/player/hud/hpicon.png")

    ix.bar.Add(function()
        local armorText, armorTextColor = hook.Run("GetArmorText", LocalPlayer())

        return math.min(LocalPlayer():Armor() / LocalPlayer():GetMaxArmor(), 1), armorText
    end, Color(30, 70, 180), nil, "armor", "90/projectparagon/ui/player/hud/kevlaricon.png")
end
