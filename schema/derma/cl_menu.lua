...
function PANEL:Init()
    ...
    self.buttons:SetSize(self:GetWide(), ScreenScale(16) * 1.75)
    self.buttons:Dock(TOP)
    self.buttons:DockPadding(ScreenScale(8), ScreenScale(8), ScreenScale(8), ScreenScale(8))
    self.buttons.Paint = function(this, width, height)
        Schema:DrawOutlinedPanel(this, width, height)
    end

    self.bottombuttons = self:Add("Panel")
    self.bottombuttons:SetSize(self:GetWide(), ScreenScale(16) * 1.75)
    self.bottombuttons:Dock(BOTTOM)
    self.bottombuttons:DockPadding(ScreenScale(8), ScreenScale(8), ScreenScale(8), ScreenScale(8))
    self.bottombuttons.Paint = function(this, width, height)
        Schema:DrawOutlinedPanel(this, width, height)
    end

    local close = self.bottombuttons:Add("ixMenuButton")
    close:SetText("return")
    close:SetFont("Font-Elements-ScreenScale8")
    close:Dock(LEFT)
    ...
    local characters = self.bottombuttons:Add("ixMenuButton")
    characters:SetText("Main Menu")
    characters:SetFont("Font-Elements-ScreenScale8")
...
function PANEL:GetStandardSubpanelSize()
    return ScrW() - self:GetPadding() * 2, ScrH() * 0.9 - self:GetPadding() * 2
end
...
function PANEL:SetupTab(name, info, sectionParent)
    ...
    if (sectionParent) then
        ...
        button:SetFont("Font-Elements-ScreenScale8")
        button:Dock(LEFT)
        button:DockMargin(0, 0, ScreenScale(2), 0)
...
function PANEL:PopulateTabs()
    ...
    self.guard:MoveToBefore(self.bottombuttons)
end
