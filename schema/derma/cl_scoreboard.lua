...
function PANEL:Init()
    ...
    self.icon:Dock(LEFT)
    self.icon.OnMousePressed = function(_, keyCode)
        if not ( keyCode == MOUSE_RIGHT ) then
            return
        end
        ...
        for _, v in pairs(menu:GetChildren()[1]:GetChildren()) do
            if ( v:GetClassName() == "Label" ) then
                v:SetFont("Font-Elements-ScreenScale8")
            end
        end
    end
...
function PANEL:Update()
    ...
    if ( self.icon.Entity ) then
        for _, v in pairs(client:GetBodyGroups()) do
            self.icon.Entity:SetBodygroup(v.id, client:GetBodygroup(v.id))
        end
    end
...
hook.Remove("CreateMenuButtons", "ixScoreboard")
